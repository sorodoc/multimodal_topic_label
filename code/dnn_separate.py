...
import multimodal_model_separate
import shelve
...
def prepare_textual_baseline(fold_nr = 5):
    ...
    for i in range(fold_nr):
        ...
        for j in range(4332):
            ...
            t_baseline[i][int(els[1])].append((els[0], float(v)))
    return t_baseline
...
folds = read_folds()
#text_base = prepare_text_baseline()
...
#    p_t_inp = calc_page_rank(w_t_inf)
    ...
    v_tr_inp = add_v_embs(v_tr_inf, t_w_embs, t_d_embs, v_embs, c_w_embs, c_d_embs)
    v_t_l = v_fold['test_l']
    v_t_inf = v_fold['test_inf']
    v_t_inp = add_v_embs(v_t_inf, t_w_embs, t_d_embs, v_embs, c_w_embs, c_d_embs)
    m1 = multimodal_model_separate.TNN()
    model1 = m1.buildModel()
    m2 = multimodal_model_separate.TNN(input_size = 1200)
    model2 = m2.buildModel2()
    ...
    for j in range(nb_epoch):
        ...
        for i in range(batches):
            ...
            loss2 += model2.train_on_batch([w_tr_inp[st2: fin2], p_tr_inp[st2: fin2]], [w_tr_l[st2: fin2]])
            count_loss1 += 1.0
            count_loss2 += 1.0
        if len(v_tr_l) % b_size1 > 0:
            ...
            count_loss1 += 1.0
        if len(w_tr_l) % b_size2 > 0:
            st1 = (len(w_tr_l) % b_size2) * -1
            loss2 += model2.train_on_batch([w_tr_inp[st1:], p_tr_inp[st1:]], [w_tr_l[st1:]])
            count_loss2 += 1.0
        print 'visual loss : ' + str(loss1 / count_loss1) + ' batches : ' + str(count_loss1) + ' batch_size : ' + str(b_size1)
        print 'textual loss : ' + str(loss2 / count_loss2) + ' batches : ' + str(count_loss2) + ' batch_size : ' + str(b_size2)
...
#    model2.fit([w_tr_inp], [w_tr_l], nb_epoch = nb_epoch, batch_size = batch_size, validation_split = 0.1)
    probs2 = model2.predict([w_t_inp, p_t_inp], batch_size = batch_size)
